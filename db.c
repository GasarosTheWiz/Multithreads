...
{   int pos; //metabliti gia tis anagkes tou return

    pthread_mutex_lock(&self->readwrite_mutex);  //lock
    ...
    pos=memtable_add(self->memtable, key, value); //metabliti edw wste na epistrafei meta to unlock
    pthread_mutex_unlock(&self->readwrite_mutex); //unlock
    return pos;						//epistrefoume to zitoumeno
}

int db_get(DB* self, Variant* key, Variant* value)
{   int pos; //metabliti opws apo panw gia to return

    pthread_mutex_lock(&self->readcount_mutex); //lock to mutex gia read
    self->readcount++;	//anebazw to count gia readers 1 
    
    if(self->readcount==1)   //an iparxei reader
       pthread_mutex_lock(&self->readwrite_mutex); //lock tous writers
    pthread_mutex_unlock(&self->readcount_mutex); //unlock tous readers

    if (memtable_get(self->memtable->list, key, value) == 1)
    {    
           pos=1;   //ekxwrisi timis
         
    }
    else
    { 
	   pos=sst_get(self->sst, key, value);  //ekxwrisi timis
    }

    pthread_mutex_lock(&self->readcount_mutex); //lock readcount gia na allaksw tin global
    self->readcount--;   //-1 readcount
     
    if(self->readcount==0)  //an pleon to readcount einai 0
       pthread_mutex_unlock(&self->readwrite_mutex);  //unlock writers mutex,mporei na ksanampei nima
    pthread_mutex_unlock(&self->readcount_mutex);  //unlock ta read

    
    return pos; //epistrofi tis timis pou ekxwri8ike
